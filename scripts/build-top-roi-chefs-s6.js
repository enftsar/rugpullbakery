...
const SNAPSHOT_FILE = path.join(ROOT, "data", "s6-bake-snapshot.json");
const OUT_JSON = path.join(ROOT, "reports", "top-roi-chefs-s6.json");
const OUT_CSV = path.join(ROOT, "reports", "top-roi-chefs-s6.csv");
...
async function main(){
  ...
  if(Number(snapshot.season)!==6) throw new Error("Snapshot is not season 6");
